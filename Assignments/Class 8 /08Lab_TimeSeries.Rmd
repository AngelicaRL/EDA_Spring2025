...
Rainfall_BR_processed <-
  ...
  #To separate one collumnwith many info in 3 different 1 
...
# Alternatively, check if there are any missing values (TRUE/FALSE)
any(BR_complete$Temperature_C)
any(BR_complete$Rainfall)
...
# Assuming you have a data frame named 'data' with columns 'date', 'temperature', and 'rainfall'
ggplot(BR_complete, aes(x = Date, y = Temperature_C)) +
  geom_line(color = "blue") +
  geom_smooth(method = "loess", color = "red", se = FALSE) +
  labs(title = "Temperature Over Time", x = "Date", y = "Temperature_C") +
  theme_minimal()

ggplot(BR_complete, aes(x = Date, y = Rainfall)) +
  geom_line(color = "green") +
  geom_smooth(method = "loess", color = "red", se = FALSE) +
  labs(title = "Rainfall Over Time", x = "Date", y = "Rainfall") +
  theme_minimal()
...
BR_temp_ts<- ts(BR_complete$Temperature_C, frequency=12, start=c(1901,1))

BR_temp_decomposeed <- stl(BR_temp_ts, s.window="periodic")
plot(BR_temp_decomposeed)
...
BR_stl_rain_ts <- as.data.frame(BR_complete$time.series) %>%
  mutate(Observed=BR_complete) 


stl(BR_complete$Rainfall, s.window="periodic")
